...
class DiyPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        print('process_item>>>>>>>>>>>>>>>>>>>>>>>')
        # 查询公司存储个数, 如果没有则存储~
        sql_count = "select count(0) from bus_user where company_name='{}'".format(item['con_name'])
        ...
        if result_count > -1:
            ...
            dict_data = {
                "con_info": {"create_date": item['create_date'], "com_name": item["con_name"],
                             "linkman": item["linkman"], "address": item["address"], "mobile": item["mobile"],
                ...
                "pro_info": {"create_date": item['create_date'], "price": item['price'], "title": item['title'],
                             ...
                             "detail": item['detail'], "units": '', "urrency": item["urrency"]}}
    ...
    def close_spider(self, spider):
        sql_id = "SELECT id FROM bus_spider_data WHERE source = 'Ralali' and TYPE = 'gongying' AND is_del = '0' AND isuse = '0' ORDER BY create_date LIMIT 1 "
